...
export default async function NewsPage() {
  ...
  const posts = await getPostsByType(PostType.NEWS, session?.user?.id);
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-5xl space-y-6 px-4 py-10 text-slate-100 sm:px-6 lg:px-8">
      <header>
        ...
        <p className="text-sm text-slate-300">Latest news content with favorite toggles.</p>
      </header>

      {posts.length === 0 ? (
        <EmptyState title="No news posts yet" description="News posts will appear when real content is published." />
      ) : (
