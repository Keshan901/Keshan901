...
export default async function BlogsPage() {
  ...
  const posts = await getPostsByType(PostType.BLOG, session?.user?.id);

  return (
    <main className="mx-auto min-h-screen w-full max-w-5xl space-y-6 px-4 py-10 text-slate-100 sm:px-6 lg:px-8">
      <header>
        <h1 className="text-3xl font-bold">Blogs</h1>
        <p className="text-sm text-slate-300">Long-form posts and updates with favorite toggles.</p>
      </header>

      {posts.length === 0 ? (
        <EmptyState title="No blog posts yet" description="Blog posts will appear when real content is published." />
      ) : (
