...
export async function GET(request: Request) {
  ...
  const rows = await prisma.hashtagSet.findMany({
    where: {
    ...
    orderBy: parsed.sort === "newest" ? { createdAt: "desc" } : { copyCount: "desc" },
    take: 100,
  });
...
export async function POST(request: Request) {
  ...
  const created = await prisma.hashtagSet.create({ data: { ...parsed.data, authorId: session.user.id } });
